...
void
pktgen_send_latency(port_info_t *info)
{
    pkt_seq_t *ppkt   = &info->seq_pkt[LATENCY_PKT];
    struct rte_mbuf *m;

    m = rte_pktmbuf_alloc(info->latency_mp);
    if (unlikely(m == NULL)) {
        pktgen_log_warning("No packet buffers found");
        return;
    }
    pktgen_packet_ctor(info, LATENCY_PKT, -1);
    rte_memcpy(rte_pktmbuf_mtod(m, void *), (uint8_t *)&ppkt->hdr, ppkt->pktSize);

    m->pkt_len  = ppkt->pktSize;
    m->data_len = ppkt->pktSize;

    pktgen_send_mbuf(m, info->pid, 0);

...
pktgen_tstamp_pointer(port_info_t *info, char *p)
{
    ...
    p += (pkt->ethType == RTE_ETHER_TYPE_IPV4) ? sizeof(struct rte_ipv4_hdr)
                                               : sizeof(struct rte_ipv6_hdr);

    p += (pkt->ipProto == PG_IPPROTO_UDP) ? sizeof(struct rte_udp_hdr)
...
pktgen_tstamp_inject(port_info_t *info)
{
    pkt_seq_t *pkt = &info->seq_pkt[LATENCY_PKT];
    rte_mbuf_t *mbuf;
    ...
    if (mbuf) {
        ...
        printf("\n");
        pg_pktmbuf_dump(stdout, mbuf, pkt->pktSize);
    }
    return mbuf;
}
...
#endif
    if (flags & LATENCY_PKT_SEND)
        pktgen_send_latency(info);

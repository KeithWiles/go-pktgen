                        ...
                        int socket_id, int cache_size)
{
    ...
    if (pktgen.verbose)
        pktgen_log_info("    Create: '%-*s' - Memory used (MBUFs %6u x size %6u) = %6lu KB",
...
pktgen_config_ports(void)
{
    ...
    RTE_ETH_FOREACH_DEV(pid)
    {
        ...
        info->latency_rate_cycles   = rte_get_timer_hz() / (1000 / info->latency_rate);
        ...
        info->special_mp =
            pktgen_mbuf_pool_create("Special TX", pid, 0, MAX_SPECIAL_MBUFS, sid, 0);
